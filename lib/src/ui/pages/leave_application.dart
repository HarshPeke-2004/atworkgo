import 'package:atworkgo/src/ui/constants/colors.dart';
import 'package:atworkgo/src/ui/widgets/loader_dialog.dart';
...
class LeaveApplicationWidgetState extends State<LeaveApplicationWidget>
  ...
  List<String> leaveType = ["Medical Leave", "Annual Leave", "Casual Leave"];

  List<String> leaveKeys = ["ml", "al", "cl"];
  ...
  void _getManager() async {
    _userRef.child(widget.user.uid).child("manager").once().then((
      DatabaseEvent event,
    ) {
      final managerID = event.snapshot;
      _managerRef.child(managerID.value as String).once().then((
            ...
            child: Builder(
              builder: (context) => Form(
                ...
                child: Column(
                  ...
                  children: [
                    Center(child: Text("Available Leaves")),
                    Padding(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () {
                              DatePicker.showDatePicker(
                                context,
                                theme: DatePickerTheme(containerHeight: 250.0),
                    ...
                    Container(
                      child: Text('Number of days on leave: $leavesCount'),
                    ),
                    Container(
                    ...
                    Column(
                      children: List.generate(leaveType.length, (index) {
                        return Card(
                          elevation: 2,
                          ...
                          child: RadioListTile<int>(
                            ...
                            onChanged: (value) {
                              ...
                              print(
                                "Selected: ${leaveType[index]}  index: $index",
                              );
                            },
                          ),
                        );
  ...
  bool _validateData(BuildContext context) {
    ...
    if (leaveIndex == -1) {
      SnackBar error = new SnackBar(
        content: Text(
          "Please select type of leave valid dates",
          style: TextStyle(color: Colors.white),
        ),
        backgroundColor: Colors.red,
      );
